    ...
    if ( pCube == NULL ) // solve the property
    {
        ...
        if (p->pPars->fusetoFrame >= k - 1) {
            ...
            Saig_ManForEachLo( p->pAig, pObj_fuse, i )
            {
                ...
                //Enforce latch output[i] of fast == latch output[i] of slow
                //using Tseytin Transformation,
                //(a \/ -b) /\ (-a \/ b)
                Vec_IntClear( vLits_fuse );
                Lit_fuse0 = Abc_Var2Lit( Pdr_ObjSatVar(p, k, 2, Aig_ManLo(p->pAig, i)), 0 );
                Vec_IntPush( vLits_fuse, Lit_fuse0 );
                Lit_fuse1 = Abc_Var2Lit( Pdr_ObjSatVar(p, k, 2, Aig_ManLo(p->pAig, i + numLodiv2)), 1 );
                Vec_IntPush( vLits_fuse, Lit_fuse1 );
                ...
                Vec_IntClear( vLits_fuse );
                Lit_fuse0 = Abc_Var2Lit( Pdr_ObjSatVar(p, k, 2, Aig_ManLo(p->pAig, i)), 1 );
                Vec_IntPush( vLits_fuse, Lit_fuse0 );
                Lit_fuse1 = Abc_Var2Lit( Pdr_ObjSatVar(p, k, 2, Aig_ManLo(p->pAig, i + numLodiv2)), 0 );
                Vec_IntPush( vLits_fuse, Lit_fuse1 );
                RetValue_fuse = sat_solver_addclause( pSat, Vec_IntArray(vLits_fuse), Vec_IntArray(vLits_fuse)+Vec_IntSize(vLits_fuse) );
                assert(RetValue_fuse);
            }
