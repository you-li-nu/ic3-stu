...
//    pPars->iOutput        =      -1;  // zero-based output number
    ...
    pPars->fVerbose       =       1;  // verbose output //youl
    pPars->fVeryVerbose   =       1;  // very verbose output //youl
    pPars->youlVerbose    =       1;  // very verbose output //youl
...
//    pCubeMin = Pdr_SetDup( pCube );
    ...
    if ( p->pPars->fSimpleGeneral )//comment: default 0
    {
    ...
    keep = p->pPars->fSkipDown ? NULL : Hash_IntAlloc( 1 );//comment: default 1

    // perform generalization
    if ( !p->pPars->fSkipGeneral )//comment: default 0
    {
    ...
    //        i = j;
            ...
            int Lo0Var = Pdr_ObjSatVar(p, k, 2, Aig_ManLo(p->pAig, 0));
            ...
            if ( p->pPars->fSkipDown )//comment: default 1
            ...
            // success - update the cube
            pCubeMin = Pdr_SetCreateFrom( pCubeTmp = pCubeMin, i );//comment: remove Lit i from pCubeMin
            pCubeMin = Pdr_SetCreateFrom( pCubeTmp = pCubeMin, k );//comment: remove Lit k from pCubeMin
            Pdr_SetDeref( pCubeTmp );
            assert( pCubeMin->nLits > 0 );
        ...
        if ( p->pPars->fTwoRounds )//comment: default 0
        for ( j = 0; j < pCubeMin->nLits; j++ )
        {
            // use ordering
    //        i = j;
...
//                    Vec_IntWriteEntry( p->vAbsFlops, i, 1 );
        ...
        Saig_ManForEachPo( p->pAig, pObj, p->iOutCur )
        {
            ...
            if ( Aig_ObjChild0(pObj) == Aig_ManConst0(p->pAig) )
                continue;
            // check if the output is trivially solved
            if ( Aig_ObjChild0(pObj) == Aig_ManConst1(p->pAig) )//comment: the wire is driven by constant 1.
            {
                if ( !p->pPars->fSolveAll )
                {
                    ...
                    if ( p->pPars->youlVerbose )
                        Abc_Print( 1, "Trivial cex found. (youl)\n" );//youl
                    return 0; // SAT
                }
            ...
            while ( 1 )
            {
                ...
                RetValue = Pdr_ManCheckCube( p, iFrame, NULL, &pCube, p->pPars->nConfLimit, 0, 1 );//comment: &pCube should store the predecessor.
                if ( RetValue == 1 )//comment: should finish
                    break;
                if ( RetValue == -1 )
                {
                    if ( p->pPars->fVerbose )
                ...
                if ( RetValue == 0 )//comment: find a new predecessor of P 
                {
